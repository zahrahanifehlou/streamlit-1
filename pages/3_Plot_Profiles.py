...
if "df_profiles" not in st.session_state:
    ...
    main_tabs = st.tabs(
        [
            "find similar compounds(profile)",
            "find similar compounds(gene)",
            "find similar compounds(pathway)",
        ]
    )
    ...
    with main_tabs[0]:
        ...
        sql_0 = "select * from umapemd where metasource=" + "'" + choix_source + "'"
        df_src_emd = pd.read_sql(sql_0, profile_conn)
        ...
        sql_profile = (
            "select * from aggcombatprofile where metasource="
            + "'"
            + choix_source
            + "'"
        )
        ...
        if len(b_list) > 0:
            ...
            df_cpd = pd.read_sql(sql_meta, meta_conn)
