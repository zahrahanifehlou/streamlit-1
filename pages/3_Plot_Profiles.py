...
if "df" not in st.session_state:
    ...
    list_cpds = df["source"] + "+" + df["batchid"]
    choix = st.selectbox("Select the Profile", list_cpds)
    source = choix.split("+")[0]
    bid = choix.split("+")[1]
    df_sel = df[(df["source"] == source) & (df["batchid"] == bid)]
...
# JCP2022_805264
    # list_source=
    x_list = [col for col in df.columns if "source" not in col and "batchid" not in col]
    # cpd_names = df.source.values
    # df_plt = df.set_index("source")
    # df_plt = df_plt[x_list].T
    # fig = px.line(df_plt, x=x_list, y=cpd_names)
    # st.plotly_chart(fig, theme="streamlit", use_container_width=True)
    conn2 = psycopg2.connect(host="192.168.2.131", port="5432", user="arno", database="ksilink_cpds", password="12345")
    sql_profile = "select * from aggprofiles where source=" + "'" + choix_source + "'"
    ...
    filter_col = [
        col for col in df_prof.columns if not col.startswith("Meta") and "source" not in col and "batchid" not in col
    ]
    ...
    df_hist["batchid"] = df_prof['batchid']
    ...
    df_keep_prof = df_prof[df_prof["batchid"].isin(df_keep["batchid"].values)]
    st.write(df_keep_prof)

    if len(df_keep_prof) < 11:
        cpd_names = df_keep_prof.batchid.values
        df_plt = df_keep_prof.set_index("batchid")
