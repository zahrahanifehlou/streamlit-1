...
def get_plate(plate_names, collection="tags"):
    ...
    for pl in plate_names:
        ...
        if (len(plates) == 0) and collection == "tags":
            raise ValueError(
                f"plate map not found. please add plate map for the plate in checkout {pl}"
            )
        res += plates
    return res
...
headers = {"X-LC-APP-Auth": key.encode("latin-1"), "Accept": "application/json"}
...
cells = st.multiselect("Select Cell lines:", df_proj["name"].unique())
pos_ctrl = ""
neg_ctrl = ""
if len(cells) > 1:
...
if rad == "yes":
    ...
    if orig == "yes":
        url_cpd = (
            "http://labcollector.ksilink.int/webservice/index.php?v=2&module=chemicals"
        )
    ...
    if orig == "no":
        ...
        inhib = st.sidebar.radio(
            "Looking for  specific inhibitors?", ["yes", "no"], key="inhib"
        )
        if inhib == "yes":
            var_text = st.text_area(
                "Enter your search", help="Name or ID separated by enter"
            )
            ...
            if cpd_inhib:
                st.write(
                    df_inhib_sel[df_inhib_sel["ext_name"].isin(cpd_inhib)].reset_index(
                        drop=True
                    )
                )
...
if not df_inhib_sel.empty:
    list_pubchemid = [
        f"'{t}'"
        for t in df_inhib_sel[df_inhib_sel["ext_name"].isin(cpd_inhib)]["pubchemid"]
    ]
...
if on:
    ...
    uploaded_files = st.multiselect(
        "Choose result directories corresponding to this assay", paths
    )
