...
if on:
    ...
    if uploaded_file:
        ...
        gene_col = st.text_input('Write the column name with gene symbol',value='symbol')
        if gene_col:
...
if not df_genes.empty:
    ...
    if choice=='CRISPR':
        ...
        for bs in unique_gene:
            bq.append("'" + bs + "'")
        sql_genes = "select gene.*,genebatchs.batchid from gene  INNER JOIN genebatchs ON  genebatchs.geneid=gene.geneid where symbol  in (" + ",".join(bq) + ")  "
