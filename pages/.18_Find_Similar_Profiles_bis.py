...
if "df_profiles" not in st.session_state:
    ...
    df_sel = cpd_pro[
        (cpd_pro["metasource"] == choix_source) & (cpd_pro["metacpdname"] == choix)
    ].head(1)
    ...
    if len(b_list_cpd) > 0 and ("ORF" in choix_source or "CRIS" in choix_source):
        ...
        df_merge = df_results_cpd.merge(
            df_genes, left_on="geneid", right_on="geneid"
        ).reset_index(drop=True)
        ...
        df_src_emd.loc[
            df_src_emd["metageneid"].isin(df_results_cpd.geneid), "color"
        ] = "similar compounds"
        choix_batch = str(df_sel["metageneid"].values[0])
        df_src_emd.loc[
            df_src_emd["metageneid"] == choix_batch, "color"
        ] = "selected compounds"
