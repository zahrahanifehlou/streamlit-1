...
if on and tp:
    ...
    uploaded_files = st.multiselect(
        "Choose result directories corresponding to this assay", paths_clean
    )
...
if on and not dl and not cbc and not tp:
    ...
    if uploaded_files:
        ...
        if len(list_df) > 0:
            ...
            if len_col != len_col2:
                st.warning(
                    f"{len_col-len_col2} out of {len_col} columns were removed due to correlation > |0.95| ",
                    icon="🚨",
                )
            ...
            if umap_on:
                ...
                if len(df_data) > 2:
                    ...
                    data_scaled = pd.concat(
                        [data_scaled, data[cols_alpha].reset_index(drop=True)], axis=1
                    )
                    import umap
                    ...
                    df_all_umap = pd.concat(
                        [df_all_umap, data[cols_alpha].reset_index(drop=True)], axis=1
                    )
                    components.html(
...
if on and cbc:
    ...
    list_df = []
    if uploaded_file:
        files = glob.glob(f"{uploaded_file}/**/*cell_by_cell.fth", recursive=True)
