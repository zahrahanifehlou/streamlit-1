...
with tab1:
    ...
    st.write(
        "Example:  SELECT cpd.pubchemid, cpd.keggid, cpd.cpdname, gene.* from cpd INNER JOIN keggcpdgene ON keggcpdgene.keggid=cpd.keggid INNER JOIN gene ON gene.geneid=keggcpdgene.geneid"
    )
...
with tab2:
    ...
    st.write(
        "Example for jump and crispr:  SELECT * from aggprofile where metasource='CRISPER'"
    )
