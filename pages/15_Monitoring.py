...
fig = px.histogram(
    ...
    x="acquisitionDate",
    y="size",
    title="Storage activities",
    color="project",
    template="plotly_dark",
)
fig.update_traces(xbins_size="M2")
fig.update_xaxes(showgrid=True, ticklabelmode="period", dtick="M2", tickformat="%b\n%Y")
fig.update_layout(bargap=0.1)
st.plotly_chart(fig, theme="streamlit", use_container_width=True)

dfproj = (
    dfSizes[["project", "size", "serverPath"]]
    .groupby(["project", "serverPath"])
    .sum()
    .reset_index()
)
