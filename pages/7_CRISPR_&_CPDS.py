...
sql_batch = (
    "select batchs.batchid,cpd.keggid from cpd  inner join batchs on batchs.pubchemid=cpd.pubchemid where keggid  in ("
    + ",".join(bq)
    + ")"
)
...
df_inter = df_ksi_prof.merge(
    df_ksi_pubchem2, left_on="Metadata_JCP2022", right_on="batchid"
).reset_index(drop=True)
# df_inter.drop('morganfp',inplace=True,axis=1)
df_join_kegg_geneid_prof = df_inter.merge(
    df_drug_meta, left_on="keggid", right_on="keggid"
).reset_index(drop=True)
...
df_join_kegg_geneid_prof_filt["target"] = (
    df_join_kegg_geneid_prof_filt["keggid"]
    + "_"
    + df_join_kegg_geneid_prof_filt["geneid"]
)
...
df_join_kegg_geneid_prof_filt = (
    df_join_kegg_geneid_prof_filt.groupby(
        ["target", "geneid", "symbol", "name", "efficacy"]
    )
    .median()
    .reset_index()
)
...
df_crispr_data_j = df_crispr.merge(
    df_crispr_data, left_on="batchid", right_on="Metadata_JCP2022"
).reset_index(drop=True)
df_crispr_data_j_agg = (
    df_crispr_data_j.groupby(["batchid", "geneid"]).median().reset_index()
)
...
df_num_cpd["efficacy"] = df_join_kegg_geneid_prof_filt["efficacy"].apply(
    lambda x: x.split(",")[0]
)
